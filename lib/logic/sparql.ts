...
/**
 * Language definition, mostly the same as the original from 'codemirror-lang-sparql'.
 */
const CustomSparqlLanguage = LRLanguage.define({
  parser: SparqlLanguage.parser.configure({
    props: [
      styleTags({
        Comment: t.comment,
        ...
        'Pname_ns Pname_ln': t.attributeName
      }),
      foldNodeProp.add({ 
...
function sparql(): LanguageSupport {
  return new LanguageSupport(CustomSparqlLanguage)
}
